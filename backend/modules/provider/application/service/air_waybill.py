...
from modules.provider.domain.usecase.air_waybill import AirWaybillUseCaseFactory
...
class AirWaybillService:
	...
	async def get_all_air_waybills(
		self, id_invoice: int, limit: int = 20, page: int = 0
	) -> list[AirWaybill] | Sequence[AirWaybill]:
		return await self.air_waybill_usecase.get_air_waybills(id_invoice, limit, page)
	...
	async def create_air_waybill(self, command: CreateAirWaybillCommand) -> AirWaybill:
		...
		print(
			"DEBUG service.create_air_waybill ->", getattr(result, "id", None), result
		)
		return result
	...
	async def update_air_waybill(self, command: UpdateAirWaybillCommand):
		return await self.air_waybill_usecase.update_air_waybill(command)
