...
PositiveDecimal = Annotated[
	Decimal, Field(gt=0, decimal_places=4, description="Must be a positive decimal")
]

NonEmptyStr = Annotated[
	str, Field(min_length=1, max_length=500, description="Cannot be empty")
]
...
YiqiFieldId = Annotated[int, Field(gt=0, description="Yiqi field ID reference")]
...
def validate_date_not_future(v: date, field_name: str) -> date:
	...
	if v > date.today():
		raise ValueError(f"{field_name} cannot be in the future. Got: {v}")
	return v
...
class YiqiInvoiceBase(BaseModel):
	...
	Fecha_emision: date = Field(
		serialization_alias="2881", description="Fecha de emisión de la factura"
	)

	Fecha_recepcion: date = Field(
		serialization_alias="5112", description="Fecha de recepción de la factura"
	)

	Mes_servicio: date = Field(
		serialization_alias="5082", description="Mes del servicio facturado"
	)
	...
	Servicio: Optional[YiqiFieldId] = Field(
		default=None, serialization_alias="6196", description="ID del servicio en Yiqi"
	)
	...
	def validate_numero_format(cls, v: str) -> str:
		...
		if len(v) < 3:
			raise ValueError(
				f"Invoice number too short: {v}. Minimum 3 characters required."
			)
	...
	def validate_dates(cls, v: Any) -> date:
		...
		if isinstance(v, str):
			...
			raise ValueError(
				f"Invalid date format: {v}. Expected: YYYY-MM-DD or DD/MM/YYYY"
			)
	...
	def validate_awb(cls, v: Optional[str]) -> Optional[str]:
		...
		if len(awb_digits) not in [8, 11]:
			raise ValueError(
				f"AWB must be 8 or 11 digits long: {v} (got {len(awb_digits)} digits)"
			)
	...
	def validate_items(cls, v: Optional[int]) -> Optional[int]:
		...
		if v > MAX_ITEMS:
			raise ValueError(
				f"Item count seems unreasonably high: {v}. Maximum allowed: {MAX_ITEMS}"
			)
	...
	def validate_date_relationships(self) -> "YiqiInvoiceBase":
		...
		service_year_diff = abs(self.Mes_servicio.year - self.Fecha_emision.year)
	...
	def is_same_month_service(self) -> bool:
		...
		return (
			...
			and self.Mes_servicio.month == self.Fecha_emision.month
		)
...
class UploadFileCommand(BaseModel):
	...
	file: BytesIO = Field(description="File content as BytesIO stream")

	size: PositiveInt = Field(description="File size in bytes")

	filename: NonEmptyStr = Field(description="Original filename with extension")
	...
	def validate_filename(cls, v: str) -> str:
		...
		if "." not in v:
			raise ValueError(f"Filename must have an extension: {v}")
	...
	def __init__(self, file: BytesIO, size: int, filename: str, **data):
