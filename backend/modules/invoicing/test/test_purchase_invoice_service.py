...
class TestPurchaseInvoiceService:
	...
	async def test_get_list_success(
		...
		mock_purchase_invoice_repository.get_purchase_invoice_list.assert_called_once_with(
			10, 0
		)
	...
	async def test_get_list_of_provider_success(
		...
		result = await purchase_invoice_service.get_list_of_provider(
			id_provider=provider_id, limit=10, page=0
		)
	...
	async def test_get_one_by_id_success(
		...
		# Arrange
		invoice_id = 123
		mock_purchase_invoice_repository.get_purchase_invoice_by_id.return_value = (
			sample_purchase_invoice_with_id
		)

		# Act
		result = await purchase_invoice_service.get_one_by_id(
			id_purchase_invoice=invoice_id
		)

		# Assert
		assert result == sample_purchase_invoice_with_id
		mock_purchase_invoice_repository.get_purchase_invoice_by_id.assert_called_once_with(
			invoice_id
		)
	...
	async def test_save_invoice_success(
		...
		"""
		# Arrange
		mock_purchase_invoice_repository.save_purchase_invoice.return_value = (
			sample_purchase_invoice_with_id
		)
		...
		mock_purchase_invoice_repository.save_purchase_invoice.assert_called_once_with(
	...
	async def test_create_and_save_flow(
		...
		mock_purchase_invoice_repository.save_purchase_invoice.side_effect = (
			save_side_effect
		)
...
class TestPurchaseInvoiceServiceIntegration:
	...
	async def test_full_flow_with_real_repository(
		...
		retrieved_invoice = await real_purchase_invoice_service.get_one_by_id(
			invoice_id
		)
